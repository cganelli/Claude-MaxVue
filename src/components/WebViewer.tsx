...
const WebViewer: React.FC<WebViewerProps> = ({ url = 'https://example.com', className = '' }) => {
  ...
  const handleRefresh = () => {
    ...
    const iframe = document.querySelector('#web-viewer-iframe') as HTMLIFrameElement;
    if (iframe) {
      ...
      <div className="bg-gray-100 p-4 border-b border-gray-200">
        ...
        <div className="mt-2 flex items-center space-x-2">
          <div className={`w-2 h-2 rounded-full ${
            isLoading ? 'bg-yellow-500' : hasError ? 'bg-red-500' : 'bg-green-500'
          }`}></div>
          ...
          <div className="h-full bg-red-50 flex items-center justify-center p-8">
            <div className="text-center">
              ...
              <p className="text-red-600 mb-4">
                Unable to load <span className="font-mono text-sm">{currentUrl}</span>
              </p>
