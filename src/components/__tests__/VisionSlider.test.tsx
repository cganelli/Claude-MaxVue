...
describe("Vision Slider Functionality", () => {
  ...
  it("should update vision state when slider changes", async () => {
    ...
    const visionSlider = sliders.find(
      (slider) =>
        slider.getAttribute("max") === "3.5" &&
        slider.getAttribute("step") === "0.25",
    );

    expect(visionSlider).toBeDefined();

    // Test slider change
    fireEvent.change(visionSlider!, { target: { value: "3.5" } });
