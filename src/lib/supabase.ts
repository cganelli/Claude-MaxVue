...
console.error = (...args: any[]) => {
  ...
  const shouldFilter = args.some(arg => {
    ...
    return argString.includes('user_already_exists') || 
           ...
           (argString.includes('Supabase request failed') && argString.includes('user_already_exists')) ||
           (argString.includes('Supabase request failed') && argString.includes('invalid_credentials'));
  });
...
console.log('🔍 Environment:', {
  ...
  isBoltEnvironment: window.location.hostname.includes('bolt.new') || 
...
export const refreshStorageDetection = () => hybridStorage.refreshStorageDetection();

// ✅ UTILITY FUNCTION: Wait for session to be available
export const waitForSession = async (maxAttempts: number = 10, delayMs: number = 250): Promise<any> => {
  ...
  while (attempts < maxAttempts) {
    ...
    attempts++;
    console.log(`⏳ Session not ready, attempt ${attempts}/${maxAttempts}, waiting ${delayMs}ms...`);
...
export const fetchProfile = async (userId: string, retries: number = 3) => {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      console.log(`📋 Profile fetch attempt ${attempt} result:`, { data, error });

      if (error) {
        if (error.code === 'PGRST116') {
          console.log(`⚠️ Profile not found for user: ${userId} (attempt ${attempt}/${retries})`);
          if (attempt === retries) {
      ...
      console.error(`💥 Exception during profile fetch (attempt ${attempt}):`, err);
      if (attempt === retries) {
