...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const fallbackTimeout = setTimeout(() => {
      if (mounted) {
        console.warn(
          "⚠️ Auth still loading after 5s, forcing isLoading = false",
        );
        setIsLoading(false);
      }
    }, 5000);
  ...
  const handleUserSession = async (
    ...
    try {
      ...
      if (profile) {
        ...
        console.log(
          "ℹ️ User may be in registration process, keeping session active",
        );
  ...
  const register = async (
    ...
    try {
      ...
      console.log(
        "🔍 Final storage info after registration:",
        getStorageInfo(),
      );
